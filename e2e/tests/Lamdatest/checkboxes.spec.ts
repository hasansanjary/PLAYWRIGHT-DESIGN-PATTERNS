...
test('checkboxes', async ({page}) => {
    ...
    const multipleCheckboxesLocator: Locator = await page.locator("//input[@class='mr-8' and @type='checkbox']");

    console.log("Total number of checkboxes: ", + await multipleCheckboxesLocator.count());

    const count = await multipleCheckboxesLocator.count();
    for(let idx = 0; idx < count; idx++){
        await multipleCheckboxesLocator.nth(idx).check();
        expect(multipleCheckboxesLocator.nth(idx)).toBeChecked();
    }
